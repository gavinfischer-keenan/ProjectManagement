  ...
  const sections = useMemo(() => {
    ...
    return topParents
      .map((p) => {
        ...
        const completed = leaves.filter((c) => c.dateFinished || c.status === 'Completed').length;
        const inProg    = leaves.filter((c) => c.status === 'In Progress' && !c.dateFinished).length;
        const lateCount = leaves.filter((c) => isLate(c.targetDateFinish, c.dateFinished)).length;
        const notStart  = Math.max(0, total - completed - inProg);
        ...
        return {
          ...
          total, completed, inProg, lateCount, notStart, pct,
          start:  starts[0]  || null,
          ...
          <table className="dash-sections-table">
            ...
            <tbody>
              {sections.map((s) => {
                ...
                return (
                  <tr key={s.id} className={`dash-section-row ${s.pct === 100 ? 'dash-section-done' : ''}`}>
                    ...
                    <td className="dash-cell-num" style={{ color: '#10b981' }}>{s.completed}</td>
                    <td className="dash-cell-num" style={{ color: '#3b82f6' }}>{s.inProg}</td>
                    <td className="dash-cell-num" style={{ color: s.lateCount > 0 ? '#ef4444' : '#64748b' }}>
